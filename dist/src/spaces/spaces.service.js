...
let SpacesService = SpacesService_1 = class SpacesService {
    ...
    async updateSpace(id, dto) {
        ...
        if (dto.parentSpaceId) {
            ...
            if (parentSpace.id === space.id) {
                throw new common_1.BadRequestException('A space cannot be its own parent.');
            }
            if (parentSpace.type !== 'SUPER') {
                throw new common_1.BadRequestException('A parent space must be a SUPER space. A REGULAR space cannot be a parent.');
            }
        }
        if (dto.type === 'REGULAR') {
            const childCount = await this.prisma.space.count({ where: { parentSpaceId: space.id } });
            if (childCount > 0) {
                throw new common_1.BadRequestException('Remove all child spaces before converting this space to REGULAR.');
            }
        }
    ...
    async getSpaceMembers(spaceId, options = {}) {
        const space = await this.prisma.space.findUnique({
            where: { id: spaceId },
        });
        if (!space) {
    ...
    async searchAddableUsers(spaceId, search) {
        const space = await this.prisma.space.findUnique({
            where: { id: spaceId },
            select: { id: true },
        });
    ...
    manageableRolesFor(isAdmin, actorRoleCode) {
        if (isAdmin) {
            return [
                client_1.RoleCode.ORGANISER,
                client_1.RoleCode.CO_ORGANISER,
        ...
        if (actorRoleCode === client_1.RoleCode.ORGANISER) {
            return [client_1.RoleCode.CO_ORGANISER, client_1.RoleCode.MEMBER, client_1.RoleCode.VOLUNTEER];
        }
        return [];
    }
    async resolveActor(spaceId, actingUserId) {
        const roles = await this.prisma.userRole.findMany({
            where: { userId: actingUserId },
            include: { role: true },
        });
        const isAdmin = roles.some((r) => r.role.code === client_1.RoleCode.ADMIN);
        const spaceRole = roles.find((r) => r.spaceId === spaceId)?.role.code ?? null;
        return { isAdmin, spaceRole };
    }
    async addSpaceMember(spaceId, actingUserId, userId, roleCode) {
        const space = await this.prisma.space.findUnique({
            where: { id: spaceId },
        });
        if (!space) {
        ...
        const { isAdmin, spaceRole } = await this.resolveActor(spaceId, actingUserId);
        const allowed = this.manageableRolesFor(isAdmin, spaceRole);
        if (!allowed.includes(roleCode)) {
            throw new common_1.ForbiddenException('You are not allowed to assign this role in this space');
        }
        ...
        if (existing) {
            throw new common_1.ConflictException('User is already a member of this space');
        }
    ...
    async updateSpaceMemberRole(spaceId, userRoleId, actingUserId, roleCode) {
        const space = await this.prisma.space.findUnique({
            where: { id: spaceId },
        });
        if (!space) {
        ...
        if (!membership || membership.spaceId !== spaceId) {
            throw new common_1.NotFoundException('Member not found in this space');
        }
        const { isAdmin, spaceRole } = await this.resolveActor(spaceId, actingUserId);
        const allowed = this.manageableRolesFor(isAdmin, spaceRole);
        if (!allowed.includes(membership.role.code) ||
            !allowed.includes(roleCode)) {
            throw new common_1.ForbiddenException('You are not allowed to change this member’s role');
        }
        if (membership.role.code === roleCode) {
            return { message: 'No change', member: membership };
        }
        if (space.createdBy === membership.userId &&
            roleCode !== client_1.RoleCode.ORGANISER) {
            throw new common_1.BadRequestException('The space creator must remain the Organiser');
        }
        const role = await this.prisma.role.findFirst({
            where: { code: roleCode, scope: 'SPACE' },
        });
    ...
    async removeSpaceMember(spaceId, userRoleId, actingUserId) {
        const space = await this.prisma.space.findUnique({
            where: { id: spaceId },
        });
        if (!space) {
        ...
        if (space.createdBy === membership.userId) {
            throw new common_1.BadRequestException('The space creator cannot be removed from the space');
        }
        const { isAdmin, spaceRole } = await this.resolveActor(spaceId, actingUserId);
        const allowed = this.manageableRolesFor(isAdmin, spaceRole);
        if (!allowed.includes(membership.role.code)) {
            throw new common_1.ForbiddenException('You are not allowed to remove this member');
        }
        await this.prisma.userRole.delete({ where: { id: userRoleId } });
        return { message: 'Member removed' };
    }
    async enforceRoleLimits(space, roleCode) {
        if (roleCode === client_1.RoleCode.ORGANISER) {
            const count = await this.prisma.userRole.count({
                where: { spaceId: space.id, role: { code: client_1.RoleCode.ORGANISER } },
            });
            if (count >= 1) {
                throw new common_1.BadRequestException('Space already has an Organiser. Only one Organiser is allowed per space.');
            }
        }
        if (roleCode === client_1.RoleCode.CO_ORGANISER) {
            const count = await this.prisma.userRole.count({
                where: { spaceId: space.id, role: { code: client_1.RoleCode.CO_ORGANISER } },
            });
    ...
    async createSubSpaceRequest(userId, dto) {
        ...
        const typesNeedingSubSpace = [
            sub_space_request_dto_1.SubSpaceRequestType.JOIN_SUPER,
            sub_space_request_dto_1.SubSpaceRequestType.CONVERT_AND_JOIN,
            sub_space_request_dto_1.SubSpaceRequestType.REMOVE_CHILD,
            sub_space_request_dto_1.SubSpaceRequestType.REMOVE_PARENT,
        ];
        const isSelfConversion = requestType === sub_space_request_dto_1.SubSpaceRequestType.CONVERT_TO_SUPER ||
            requestType === sub_space_request_dto_1.SubSpaceRequestType.CONVERT_TO_REGULAR;
        if (typesNeedingSubSpace.includes(requestType)) {
            ...
            if (subSpaceId === targetSpaceId) {
                throw new common_1.BadRequestException('A space cannot be a sub-space of itself');
            }
            if (requestType === sub_space_request_dto_1.SubSpaceRequestType.JOIN_SUPER ||
                requestType === sub_space_request_dto_1.SubSpaceRequestType.CONVERT_AND_JOIN) {
                if (subSpace.parentSpaceId) {
                    throw new common_1.ConflictException('This space already belongs to a super space');
                }
                let ancestorId = targetSpace.parentSpaceId;
                const visited = new Set();
                while (ancestorId) {
                    if (ancestorId === subSpaceId) {
                        throw new common_1.BadRequestException('This assignment would create a cycle in the space hierarchy');
                    }
            ...
            if (requestType === sub_space_request_dto_1.SubSpaceRequestType.REMOVE_CHILD ||
                requestType === sub_space_request_dto_1.SubSpaceRequestType.REMOVE_PARENT) {
                if (subSpace.parentSpaceId !== targetSpaceId) {
                    throw new common_1.BadRequestException('This space is not a child of the target super space');
                }
            }
        }
        if (requestType === sub_space_request_dto_1.SubSpaceRequestType.JOIN_SUPER) {
        ...
        else if (requestType === sub_space_request_dto_1.SubSpaceRequestType.CONVERT_TO_REGULAR) {
            if (targetSpace.type !== 'SUPER') {
                throw new common_1.BadRequestException('Target space is already a REGULAR space');
            }
            const childCount = await this.prisma.space.count({
                where: { parentSpaceId: targetSpaceId },
            });
            if (childCount > 0) {
                throw new common_1.ConflictException('Remove all child spaces before converting this space back to REGULAR');
            }
        }
        ...
        const updated = await this.prisma.$transaction(async (tx) => {
            ...
            if (approved) {
                switch (request.requestType) {
                    case sub_space_request_dto_1.SubSpaceRequestType.CONVERT_TO_SUPER: {
                    ...
                    case sub_space_request_dto_1.SubSpaceRequestType.CONVERT_AND_JOIN: {
                        await tx.space.update({
                            where: { id: request.targetSpaceId },
                            data: { type: 'SUPER' },
                        });
                    ...
                    case sub_space_request_dto_1.SubSpaceRequestType.JOIN_SUPER: {
                        if (request.subSpaceId) {
                            await tx.space.update({
                                where: { id: request.subSpaceId },
                                data: { parentSpaceId: request.targetSpaceId },
                    ...
                    case sub_space_request_dto_1.SubSpaceRequestType.REMOVE_CHILD:
                    case sub_space_request_dto_1.SubSpaceRequestType.REMOVE_PARENT: {
                        if (request.subSpaceId) {
                            await tx.space.update({
                                where: { id: request.subSpaceId },
                    ...
                    case sub_space_request_dto_1.SubSpaceRequestType.CONVERT_TO_REGULAR: {
                        const childCount = await tx.space.count({
                            where: { parentSpaceId: request.targetSpaceId },
                        });
                        if (childCount > 0) {
                            throw new common_1.ConflictException('This space still has child spaces and cannot be converted to REGULAR');
                        }
                        await tx.space.update({
                            where: { id: request.targetSpaceId },
                            data: { type: 'REGULAR' },
