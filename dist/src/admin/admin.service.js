...
let AdminService = AdminService_1 = class AdminService {
    ...
    async approveSpace(id, adminId) {
        const space = await this.prisma.space.findUnique({ where: { id } });
        if (!space) {
            throw new common_1.NotFoundException('Space not found');
        }
        if (space.status === client_1.SpaceStatus.APPROVED || space.status === client_1.SpaceStatus.ACTIVE) {
            throw new common_1.BadRequestException('Space is already approved');
        }
        const updated = await this.prisma.space.update({
            where: { id },
            data: {
                status: client_1.SpaceStatus.APPROVED,
                approvedAt: new Date(),
                approvedBy: adminId,
                rejectedAt: null,
    ...
    async rejectSpace(id, adminId, reason) {
        const space = await this.prisma.space.findUnique({ where: { id } });
        if (!space) {
            throw new common_1.NotFoundException('Space not found');
        }
        const updated = await this.prisma.space.update({
            where: { id },
            data: {
                status: client_1.SpaceStatus.REJECTED,
                rejectedAt: new Date(),
                approvedBy: adminId,
    ...
    async updateSpaceStatus(id, status, adminId) {
        const space = await this.prisma.space.findUnique({ where: { id } });
        if (!space) {
            throw new common_1.NotFoundException('Space not found');
        }
        if (!Object.values(client_1.SpaceStatus).includes(status)) {
            throw new common_1.BadRequestException(`Invalid space status: ${status}`);
        }
        const updated = await this.prisma.space.update({
            where: { id },
            data: { status },
