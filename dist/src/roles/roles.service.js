...
let RolesService = RolesService_1 = class RolesService {
    ...
    async assignRole(dto, assignedBy) {
        ...
        if (role.scope === client_1.RoleScope.PLATFORM && (dto.spaceId || dto.eventId)) {
            throw new common_1.BadRequestException('Platform roles cannot have a space_id or event_id');
        }
        if (role.scope === client_1.RoleScope.SPACE) {
            if (!dto.spaceId) {
                throw new common_1.BadRequestException('Space roles must have a space_id');
            }
            if (dto.eventId) {
                throw new common_1.BadRequestException('Space roles cannot have an event_id');
            }
        }
        if (role.scope === client_1.RoleScope.EVENT) {
            if (!dto.eventId) {
                throw new common_1.BadRequestException('Event roles must have an event_id');
            }
            if (dto.spaceId) {
                throw new common_1.BadRequestException('Event roles cannot have a space_id');
            }
